...
const Navigation = () => {
    return (
        <nav className="flex flex-row justify-between items-center pt-3 px-6 h-14 bg-gray-900 md:h-[74px] lg:w-[300px] lg:min-h-screen lg:h-[1069px]  lg:flex-col lg:rounded-r-xl ">
            ...
            <NavLink to="/budget" className={({ isActive }) => isActive? "flex flex-col justify-center items-center h-11 w-16 rounded-t-xl bg-[#F8F4F0] border-b-5 border-[#277C78] md:w-[104px] md:h-16  lg:w-[276px] lg:border-b-0 lg:border-l-5 lg:rounded-t-none lg:rounded-r-xl": ""}>
                {({isActive}) => (
                    <div className="md:flex md:flex-col md:items-center lg:flex-row  lg:w-full lg:pl-4">
                        <img 
                        ...
                        className="md:mb-2 lg:mr-4 lg:mb-0" 
                        />
                        {isActive && <p className="hidden md:flex md:text-xs md:font-bold md:text-center lg:text-base ">Budget</p>}
                    </div>
